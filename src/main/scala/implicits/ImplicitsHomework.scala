...
import implicits.ImplicitsHomework.SuperVipCollections4s.syntax.GetSizeScoreOps
...
object ImplicitsHomework {
  ...
  object SuperVipCollections4s {
    ...
    object instances {
      ...
      implicit val mapIterate: Iterate2[Map] = new Iterate2[Map] {
        override def iterator1[K,V](m: Map[K,V]): Iterator[K] = m.keys.iterator
        override def iterator2[K,V](m: Map[K,V]): Iterator[V] = m.values.iterator
      }

      implicit val packedMultiMapIterate: Iterate2[PackedMultiMap] = new Iterate2[PackedMultiMap] {
        override def iterator1[K,V](p: PackedMultiMap[K,V]): Iterator[K] =
          (for ((k,v) <- p.inner
                ) yield k).iterator
        override def iterator2[K,V](p: PackedMultiMap[K,V]): Iterator[V] =
          (for ((k,v) <- p.inner
                ) yield v).iterator
      }

      implicit val intSizeScore: GetSizeScore[Int] = x =>  4
      implicit val longSizeScore: GetSizeScore[Long] = x =>  8
      implicit val charSizeScore: GetSizeScore[Char] = x => 2
      implicit val byteSizeScore: GetSizeScore[Byte] = x =>  1
      implicit val stringSizeScore: GetSizeScore[String] = x => 12 + x.length * 2
  ...
  object MyTwitter {
    ...
    def createTwitCache(maxSizeScore: SizeScore): TwitCache = new TwitCache {
      implicit val fbiNoteSizeScore: GetSizeScore[FbiNote] = {
        note: FbiNote =>
      ...
      val mbc = new MutableBoundedCache[Long, Twit](maxSizeScore)
      def put(twit:Twit): Unit = mbc.put(twit.id, twit)
      def get(id:Long): Option[Twit] = mbc.get(id)
    }
  }
}
